...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    N, D = x.shape
    H = prev_h.shape[1]
    i = sigmoid(prev_h @ Wh[:, :H] + x @ Wx[:, :H] + b[:H])
    f = sigmoid(prev_h @ Wh[:, H:2*H] + x @ Wx[:, H:2*H] + b[H:2*H])
    o = sigmoid(prev_h @ Wh[:, 2*H:3*H] + x @ Wx[:, 2*H:3*H] + b[2*H:3*H])
    g = np.tanh(prev_h @ Wh[:, 3*H:4*H] + x @ Wx[:, 3*H:4*H] + b[3*H:4*H])
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    df_tmp = df * f * (1 - f)
    do_tmp = do * o * (1 - o)
    dg_tmp = dg * (1 - np.square(g))
    
    da = np.hstack((di_tmp, df_tmp, do_tmp, dg_tmp))
